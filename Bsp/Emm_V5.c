...
/*
 * STM32H7 的 DMA1 不能访问 DTCM，而原例程的静态发送缓冲区可能位于
 * DTCM。步进命令很短，改用阻塞发送还能保证多电机命令严格串行。
 */
static HAL_StatusTypeDef Emm_V5_UART_TransmitBlocking(UART_HandleTypeDef *huart,
                                                       uint8_t *data,
                                                       uint16_t len)
{
  return HAL_UART_Transmit(huart, data, len, 20U);
}

#define HAL_UART_Transmit_DMA(huart, data, len) \
        Emm_V5_UART_TransmitBlocking((huart), (data), (len))
